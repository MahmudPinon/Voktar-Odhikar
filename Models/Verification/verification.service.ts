...
import { VerificationDTO, VerificationDisDTO, VerificationIndDTO, VerificationIndustryandDistributorDTO } from './verification.dto';
...
export class VerificationService {
  ...
  async addVerificationInd(verificationInfo: VerificationIndDTO,id:number): Promise<VerificationEntity> {
    try {
      // Check for duplicate entry
      const existingVerificationYes = await this.verificationRepo.findOne({
        where: { license_number: verificationInfo.license_number,verified: 'Yes' },
      ...
      const profile = await this.profileService.getProfileByUserIdAndLicenseNumberIND(
        id,
        verificationInfo.license_number
      );
        console.log("Pro: ",profile);
